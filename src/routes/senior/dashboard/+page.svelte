<script>
	...
	async function takeSampleCall() {
		if (isCalling) return;
		isCalling = true;
		sampleCallMessage = 'Initiating check-in call with Bland AI...';

		try {
			const apiKey = import.meta.env.VITE_BLAND_AI_API_KEY;
			...
			const response = await fetch('https://api.bland.ai/v1/calls', {
				...
				body: JSON.stringify({
					phone_number: "+1234567890", // Update with target test number
					task: "You are a warm, caring assistant checking in on a senior citizen. Ask about their day and confirm if they took their meds. If they mention falling or feeling sick, calmly notify them you are alerting their caregiver. Keep responses concise and supportive.",
					voice: "rachel", 
					webhook: webhookUrl
				})
			});
			...
			if (response.ok) {
				sampleCallMessage = '✓ Call initiated! Check your phone.';
			} else {
				sampleCallMessage = '⚠️ Call failed to dispatch. Check API credentials.';
			}
		} catch (err) {
			console.error(err);
			sampleCallMessage = '⚠️ Connection error contacting Bland AI.';
		} finally {
			isCalling = false;
		}
	}
	...
	<div class="main-area">
		...
		<main class="content">
			...
			<section class="welcome">
				<div>
					<p class="section-label">YOUR DAY WITH VCARE</p>
					<h1>
						Good morning,
			...
			<section class="call-banner">
				...
				<div class="call-info">
					<p class="lime-label">YOUR NEXT VCARE CHECK-IN</p>
					<h2>
						Today at
						<span>6:00 PM</span>
					</h2>
			...
			<section class="top-grid">
				...
				<article class="panel medicines-panel">
					<div class="panel-header">
						...
						<div class="panel-icon medicine-panel-icon">✚</div>
					</div>
						...
						{#each medicines as medicine}
							...
							>
								<div class="medicine-time">
									<strong>{medicine.time.split(' ')[0]}</strong>
									<span>{medicine.time.split(' ')[1]}</span>
								</div>

								<div class="medicine-pill-icon">💊</div>
								...
								{#if medicine.status === 'taken'}
									<div class="status-complete">✓ Taken</div>
								{:else}
									<button
					...
					<div class="panel-footer">
						...
						<button class="add-button">＋ Add medicine</button>
					</div>
				</article>
				...
				<article class="panel mood-panel">
					<div class="panel-header">
						<div>
							<p class="section-label">A LITTLE CHECK-IN</p>
							<h2>How are you feeling today?</h2>
							<span class="panel-subtitle">
								Let Vcare know how you're doing.
							</span>
						</div>
						<div class="panel-icon mood-icon">♡</div>
					</div>
					...
					<div class="moods">
						...
						>
							<span class="face green-face">☺</span>
							<strong>Good</strong>
						</button>
						...
						>
							<span class="face yellow-face">●</span>
							<strong>Okay</strong>
						</button>
						...
						<button
							...
							onclick={() => setMood('low')}
						>
							<span class="face red-face">☹</span>
							<strong>Not great</strong>
						</button>
					</div>

					<div class="mood-result">
						<div class="mood-result-icon">♥</div>
						<div>
							<strong>
								Today's check-in:
								{mood === 'good'
							...
							<span>Vcare will remember this for your next call.</span>
						</div>
					</div>
				</article>
			</section>
			...
			<section class="middle-grid">
				...
				<article class="panel routine-panel">
					<div class="panel-header">
						...
						<button class="outline-button">＋ Add reminder</button>
					</div>
					...
					<div class="timeline">
						<div class="timeline-row">
							...
							<div class="timeline-icon medicine-bg">💊</div>
							<div class="timeline-content">
								<strong>Morning Medicine</strong>
								<span>1 tablet</span>
							</div>
							<div class="timeline-status done">✓ Taken</div>
						</div>

						{#each reminders as reminder}
							<div class="timeline-row">
								<div class="timeline-time">
									<strong>{reminder.time.split(' ')[0]}</strong>
									<span>{reminder.time.split(' ')[1]}</span>
								</div>
								<div class="timeline-line">
									<i
								...
								>
									{reminder.status === 'done' ? '✓ Done' : '• Upcoming'}
								</div>
							</div>
						{/each}
						...
						<div class="timeline-row">
							...
							<div class="timeline-icon vcare-bg">☎</div>
							<div class="timeline-content">
				...
				<article class="panel calls-panel">
					<div class="panel-header">
						<div>
							<p class="section-label">YOUR CONVERSATIONS</p>
							<h2>Recent Vcare calls</h2>
							<span class="panel-subtitle">
								Your latest check-ins with Vcare.
							</span>
						</div>
						<button class="small-link">View all</button>
					</div>
							...
							<div class="call-row">
								...
								<div class="call-row-content">
									<div class="call-row-top">
										<div>
											<strong>{call.date}, {call.time}</strong>
											<span>Daily Vcare check-in</span>
										</div>
										<span
											class="call-badge"
			...
			<section class="lower-grid">
				...
				<article class="panel care-panel">
					<div class="panel-header">
						<div>
							<p class="section-label">YOUR PEOPLE</p>
							<h2>Your Care Circle</h2>
							<span class="panel-subtitle">
								People you trust and want to keep close.
							</span>
						</div>
						<div class="panel-icon care-icon">♡</div>
					</div>
					...
					<div class="care-note">
						<span>♡</span>
						<p>
							Vcare can keep your chosen people informed when something
				...
				<article class="panel alerts-panel">
					<div class="panel-header">
						...
						<div class="panel-icon alert-icon">!</div>
					</div>
			...
			<section class="closing-card">
				...
				<div>
					<h2>Vcare is here for you, every day.</h2>
					<p>We call. We listen. We remember.</p>
				</div>
...
<style>
	...
	.logo {
		...
		place-items: center;
		font-size: 22px;
		box-shadow: 0 8px 18px rgba(13, 114, 73, 0.15);
	}
	...
	.nav-link.active {
		background: linear-gradient(90deg, #ecf2c9, #f6f4dc);
		color: #166144;
	}
	...
	.sample-card {
		...
		border-radius: 20px;
		background: linear-gradient(145deg, #fffdf5, #f5f3df);
	}
	...
	.call-banner {
		...
		padding: 30px 34px;
		border-radius: 28px;
		background: linear-gradient(110deg, #075f40, #08794c 60%, #086a45);
		color: white;
		display: grid;
		grid-template-columns: 110px 1fr 280px;
		align-items: center;
		gap: 28px;
		box-shadow: 0 16px 35px rgba(11, 90, 58, 0.13);
	}
	...
	.panel {
		...
		box-shadow: 0 7px 25px rgba(67, 52, 29, 0.035);
	}
	...
	.moods button.chosen {
		border-color: #8fab66;
		background: #f4f7df;
		box-shadow: 0 0 0 2px rgba(118,157,71,.07);
	}
	...
	.closing-card {
		...
		padding: 22px 28px;
		border-radius: 23px;
		background: linear-gradient(90deg, #eef2d5, #f6f1df);
		display: flex;
		align-items: center;
		gap: 15px;
	}
	...
	@media (max-width: 760px) {
		...
		.timeline-row {
			grid-template-columns: 47px 14px 34px 1fr;
		}
